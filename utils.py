...
# ------------------------------
# construct_topology()
# @purpose: create the whole layer by using each pieces
# ------------------------------
def construct_topology(graph_name, each_part, Node2Part):
    # using weighted graph to store every thing
...
# ------------------------------
# save_topology()
# @purpose: draw a graph based on an adj
# ------------------------------
def save_topology(adj, path, graph_name, link_possibility):
    ...
    for src in range(len(adj_list)):
        ...
        for dst in range(len(adj_list[src])):
            if adj_list[src][dst] >= link_possibility:
...
# @input:
#        input_ : input vector
#        output_size : output vector shape
#        scope : name
#        stddev : 0.02
#        bias_start : 0.0
#        with_w : False ~~ return Wx+B / TRUE ~~ return Wx+B & matrix & bias
# ---------------------------------------------------------------------------------------------
...
# @input:
#        input_ : input tensor
#        output_filter : output filter number
#        k_h : filter height
#        k_w : filter width
#        d_h : strides height
#        d_w : strides width
